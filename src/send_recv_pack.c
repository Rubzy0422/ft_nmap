...
/*   Updated: 2020/11/20 09:42:29 by rcoetzer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *send_recv_pack(void *scan_mem)
{
	...
	int rsock_udp = socket(AF_INET, SOCK_RAW, IPPROTO_UDP);
	int rsock_tcp = socket(AF_INET, SOCK_RAW, IPPROTO_TCP);

	if (!data || !buffer || !scan || !rsock_icmp || !rsock_udp || !rsock_tcp)
	...
	while(1)
	{
		...
		else if (result > 0) 
		{
			for (int i = 0; i < 3; i++)
			{
				...
				{
					data_size = recvfrom(poll_set[i].fd , buffer , 8192 , 0, NULL, NULL);
					if(data_size < 0)
					{
